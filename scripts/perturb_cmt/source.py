...
class CMTSource(object):
    ...
    def write_CMTSOLUTION_file(self, filename):
        ...
        with open(filename, "w") as f:
            ...
            f.write(' PDE %4i %02i %02i %02i %02i %5.2f %9.4f %9.4f %5.1f %.1f %.1f'
                    ' %s\n' % (
            ...
            f.write('time shift:%17.4f\n' % (self.time_shift))
            f.write('half duration:%14.4f\n' % (self.half_duration))
            f.write('latitude:%19.4f\n' % (self.latitude))
            f.write('longitude:%18.4f\n' % (self.longitude))
            f.write('depth:%22.4f\n' % (self.depth_in_m / 1e3))
            f.write('Mrr:%24.6E\n' % (self.m_rr)) #* 1e7,))
            f.write('Mtt:%24.6E\n' % (self.m_tt)) #* 1e7,))
            f.write('Mpp:%24.6E\n' % (self.m_pp)) #* 1e7,))
